...
This repository is a focused Parity demo for Mocha. It intentionally separates aggregate file coverage from matching-test coverage, which is the core Parity use case.
...
| --- | --- | ---: | ---: | ---: | --- |
| `code/src/greeting.js` | `code/tests/greeting.spec.js` | 70% | 40% | 2 | Overall coverage looks acceptable, but most covered lines come from incidental tests. |
| `code/src/discount.js` | `code/tests/discount.spec.js` | 90% | 90% | 2 | The matching test owns nearly all of the file's coverage. |
...
- `greeting.js`: `minimum-coverage = 70%`, `matched-coverage = 40%`, `coverage-attribution = 3|2`.
- `discount.js`: `minimum-coverage = 90%`, `matched-coverage = 90%`, `coverage-attribution = 3|2`.
